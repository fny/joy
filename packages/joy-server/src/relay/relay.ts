/**
 * Relay integration for joy-server.
...
export class RelayClient {
   ...
   *
   * The point of doing this from joy-server is purely a UX guarantee:
   ...
   * `~/foo`. joy-server always knows its homedir, so we can guarantee
   * the field is set.
   *
    ...
    for (const to of tokens) {
      try {
        const r = await fetch('https://exp.host/--/api/v2/push/send', {
          ...
          body: JSON.stringify([{ to, title, body: body || undefined, sound: 'default', data: { source: 'joy-server', timestamp: Date.now() } }]),
        });
        if (r.ok) {
  ...
  async sendSessionPushEvent(sessionId: string, kind: 'done' | 'permission' | 'question', title: string, body: string): Promise<void> {
    const body_ = JSON.stringify({ kind, title, body, data: { kind, sessionId, source: 'joy-server' } });
...
export async function createRelaySession(
  ...
  const metadata: Record<string, unknown> = { path: opts.cwd, host: hostname(), version: '0.1.0', machineId: client.creds.machineId, joy__source: 'joy-server', joy__sessionId: opts.id, joy__state: state };
  // Agent flavor drives the app's per-agent rendering (codex diff/patch views).
  // Absent → the app treats a joy-server session as claude.
...
// joy-server downtime are delivered on next startup, not silently skipped.
