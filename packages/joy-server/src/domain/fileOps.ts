// File/shell operation handlers exposed to the app for joy-server sessions.
//
...
// joy-server sessions as they do against happy-cli sessions. Request/response
...
// binaries are platform-specific and live next to joy-server in the monorepo.
...
export async function handleBash(workingDirectory: string, data: BashRequest): Promise<BashResponse> {
  ...
  try {
    const options: ExecOptions = {
      ...
      // implicitly (its process cwd IS the session dir); joy-server's daemon
