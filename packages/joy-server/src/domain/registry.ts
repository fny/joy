...
// claude spawn + relay attach), recovering them after a joy-server restart,
...
/**
 * Expand a leading ~ to the joy-server user's home directory. tmux's -c flag
...
export class SessionRegistry {
  ...
  async create(opts: CreateSessionOpts): Promise<AgentSession> {
    ...
    // YOLO mode is the default for joy-server sessions — the app drives the
    ...
    const relayPromise = this.relayClient
      ? createRelaySession(this.relayClient, { tag: `joy-server-${id}`, cwd, id })
      : null;
  ...
  async #createOpencodeSession(opts: CreateSessionOpts, id: string, cwd: string): Promise<AgentSession> {
    ...
    if (this.relayClient) {
      try {
        const rs = await createRelaySession(this.relayClient, { tag: `joy-server-${id}`, cwd, id, flavor: "opencode" });
  ...
  async #createCodexSession(opts: CreateSessionOpts, id: string, windowName: string, cwd: string): Promise<AgentSession> {
    ...
    if (this.relayClient) {
      try {
        const rs = await createRelaySession(this.relayClient, { tag: `joy-server-${id}`, cwd, id, flavor: "codex" });
        session.attachRelay(rs);
      } catch (e) {
  ...
  // ── Recovery (joy-server restart with live tmux windows) ──────────────────────
  ...
  #attachRelayAsync(session: AgentSession, afterAttach?: () => void): void {
    ...
    createRelaySession(this.relayClient, { tag: `joy-server-${session.id}`, cwd: session.cwd, id: session.id, state })
      .then(rs => {
