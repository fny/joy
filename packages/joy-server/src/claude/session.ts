...
// its own internal retries), joy-server re-sends the failed turn on this schedule
    ...
    this.#tailer = self = tailJsonl(
      ...
      (h) => {
        const what = h.kind === "parse"
          ? "transcript format unrecognized — a Claude Code update likely changed it; update joy-server"
  ...
  onTranscriptEntry(entry: Record<string, unknown>): void {
    ...
    if (Session.#KNOWN_ENTRY_TYPES.has(entryTypeForHealth)) {
      ...
      if (this.#unknownEntryStreak === 25) {
        this.#emitAgentNote(
          `transcript entries unrecognized (type "${entryTypeForHealth || "?"}" ×${this.#unknownEntryStreak}) — a Claude Code update likely changed the format; update joy-server`,
