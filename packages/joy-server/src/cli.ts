...
// joy — CLI for the joy-server daemon. Mirrors happy-cli's surface (start/stop/
// restart/status/list/doctor/install/auth/notify) but drives the joy-server
...
// store (…/.pnpm/@fny+joy-server@1.0.15_…/node_modules/@fny/joy-server). Baking THAT into a
...
async function cmdStatus(): Promise<number> {
  const s = await probe();
  if (!s) { console.log(`${bad} joy-server daemon not running`); return 1; }
  console.log(`${ok} joy-server daemon ${c.b("running")}`);
...
async function cmdStart(): Promise<number> {
  ...
  child.unref();
  process.stdout.write("starting joy-server daemon");
  for (let i = 0; i < 50; i++) {
...
function cmdAuth(): number {
  ...
  if (!existsSync(accessKey)) {
    ...
    if (isDefaultRelay()) {
      console.log(`  joy-server shares the Joy app's credentials (${c.dim(accessKey)}).`);
...
function serviceName(): string { return isDefaultRelay() ? "joy-server" : `joy-server-${joyRelayKey()}`; }
function systemdUnitPath(): string { return join(homedir(), ".config", "systemd", "user", `${serviceName()}.service`); }
const LAUNCHD_LABEL = "vip.voltai.joy-server";
...
// Historical service names (the package was joy-tmux until 2026-08-13, and
// per-relay units shipped under joy-tmux-<relayKey>). removeService() tears
// these down too, so a re-install MIGRATES the old unit away instead of
// leaving two daemons supervising the same tmux server.
const LEGACY_SERVICE_BASES = ["joy-tmux"];
const LEGACY_LAUNCHD_LABELS = ["vip.voltai.joy-tmux", "party.voltai.joy-tmux"];
...
function cmdInstall(): number {
  ...
  if (plat === "linux") {
    const unit = `[Unit]
Description=joy-server daemon (relay ${joyRelayUrl()})
...
const RELEASE_SPEC = "git+https://github.com/fny/joy.git#release&path:packages/joy-server";
function cmdUpdate(): number {
  console.log("updating @fny/joy-server from release branch…");
...
// The programmatic surface: lets other programs and agents drive joy-server
...
function help(): void {
  console.log(`${c.b("joy")} — joy-server daemon control
