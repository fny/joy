...
// joy — CLI for the joy-cli daemon. Mirrors happy-cli's surface (start/stop/
// restart/status/list/doctor/install/auth/notify) but drives the joy-cli
...
// store (…/.pnpm/@fny+joy-cli@1.0.15_…/node_modules/@fny/joy-cli). Baking THAT into a
...
async function cmdStatus(): Promise<number> {
  const s = await probe();
  if (!s) { console.log(`${bad} joy-cli daemon not running`); return 1; }
  console.log(`${ok} joy-cli daemon ${c.b("running")}`);
...
async function cmdStart(): Promise<number> {
  ...
  child.unref();
  process.stdout.write("starting joy-cli daemon");
  for (let i = 0; i < 50; i++) {
...
function cmdAuth(): number {
  ...
  if (!existsSync(accessKey)) {
    ...
    if (isDefaultRelay()) {
      console.log(`  joy-cli shares the Joy app's credentials (${c.dim(accessKey)}).`);
...
function cmdInstall(): number {
  ...
  if (plat === "linux") {
    const unit = `[Unit]
Description=joy-cli daemon (relay ${joyRelayUrl()})
...
const RELEASE_SPEC = "git+https://github.com/fny/joy.git#release&path:packages/joy-cli";
function cmdUpdate(): number {
  console.log("updating @fny/joy-cli from release branch…");
...
// The programmatic surface: lets other programs and agents drive joy-cli
...
function help(): void {
  console.log(`${c.b("joy")} — joy-cli daemon control
...
${c.b("Usage:")} joy [--relay <happy|happy-joy|joy|joy-dev|url>] <command>
  ...
  ${c.b("update")}       Update @fny/joy-cli from the repo's release branch, then reinstall + restart
