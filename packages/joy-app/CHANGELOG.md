...
- **A Codex session shows Codex's quota.** It used to show Claude's, which cannot stop it. Harnesses with no quota surface (OpenCode, Pi, Gemini) show the context reading instead.
...
- **Restore latest, per project.** In Projects, a folder that lost something offers to bring back just its newest session — for when you want the one thing you were in the middle of rather than everything.
- A session that is still running is never offered: restoring it would put two agents in one folder.
- **"Sessions" on the machine page is now "Projects"**, and the list is ordered by most recent instead of alphabetically — what you were working on was being buried under whatever started with an 'a'.
- **Automations are editable.** Name, folder, machine, prompt and trigger, from the automation's own row. The name comes first and is required now; deriving it from the first few words of the prompt gave everything a name that read like a fragment of a sentence.
- Two triggers were removed — "after a session finishes a turn" and "when the machine comes back" — along with one that never worked. What is left is **Only when I ask**, **On a schedule** and **After another automation**.
...
- **Pinned sessions read at a glance.** A pin is one line — project, then title, then its status dot — and the project never gets cut off. They are ordered by status: amber first (needs permission, or needs a hand), then green (unread), then blue (working), then grey (already seen). The order is the one you can see, because it follows the dots. Press the Pinned header to order by project name instead, if you would rather the list never moved.
...
- **Pinned sessions** sit in a section above everything, including the active block. A pin is the one row whose position you chose yourself, and the active block grows and shrinks on its own — anything below it moves. Pin from a session's long-press menu.
- **A pin is one line**: the project, then the title, then a status dot. The project never gets cut off; the title gives up width first. Pins are ordered by project name, so an agent speaking never reorders the list you built by hand — **press the Pinned header to order by state instead**: needs a decision, then finished, then working, then gone.
- **Fold a machine away from the sidebar.** Every machine divider now has a chevron: tap the row to collapse that machine, tap its name to open the machine page. A folded machine still shows how many sessions are inside and a dot if any of them is waiting on you, so nothing hides behind it.
- **Machine dividers align.** The name used to be centred between two rules, so its left edge moved with the length of the host name — three collapsed machines, three different starts. The name now sits at a fixed indent with a single rule after it.
- **Branches and line counts appear without opening each session.** A project's git status was only ever fetched when you opened one of its sessions, so most headers showed a bare project name. The header now asks for its own.
- **Settings → Appearance → Machine glyph** steps the size of the monitor icon on a machine divider, next to the identicon size.
- Still behind Settings → Features → New session list.

# Sep 10 (2) — Thinking, folded

- **An agent's thinking arrives in the chat as a collapsed row** rather than as a wall of text you scroll past. Open it when you want to see the reasoning.
- **Claude Code's "Switch model?" confirmation and effort slider are answered properly**, instead of leaving the session sitting on a prompt nobody could see.
- **Every harness starts in its own no-prompts mode**, so a new session does not stop on the first permission question the harness happens to ask.
...
- **And that signal can no longer be lost.** If the message saying a turn ended failed to reach the server — a brief connection drop — it was never sent again, and the session stayed blue for good. It is now retried on the next turn.
...
- **Session settings follow the same table.** Model and effort can be switched live where the daemon supports it (codex and opencode through the daemon, Claude through its slash commands) and are shown read-only where it does not; the status row shows effort for any harness that reports one.
- **Settings → Models trims the catalogs.** An opencode install lists a few hundred models and a picker that cycles through all of them is useless. Each harness starts on the daemon's recommended short list; switch models on or off per harness, or enable them all, with a filter for long lists. Catalogs are read from the machine you pick at the top; the choice applies to every machine, and the model a session is on is always listed.
- **An older daemon keeps its old rows.** A machine that has not been updated publishes no table, and the page falls back to exactly what that daemon accepts, so nothing appears that would be ignored.
...
- **A queued message stays queued until the turn actually ends.** Holding a message and releasing it are two different pieces of code, and only the first was fixed. The release sweep still required the live signal, so after a cold start or a reconnect it let a held message go straight into the running turn, where it appeared as a chat bubble above the answer it was queued behind.
- **The status now says when the terminal is waiting on you.** A login prompt, a “Switch model?” confirm or a codex approval each had its own bar at the top of the session, but none of them reached the status line — so the app could report a streaming reply while the terminal sat on a question nobody could see. Those now outrank everything except being unreachable: they are the only states that will not clear on their own, and they usually explain whatever else looks stuck.
- **Voice hears "done" again.** The hook that tells voice a turn ended watched only the live "working" signal, so a turn carried by the saved one — after a cold start or a reconnect — or one that ended out of compaction or a retry never produced a "ready". Voice simply sat waiting. It now watches the same turn state everything else does.
...
- **Settings → Machines → Storage shows what every session leaves behind, and deletes it.** Each machine's daemon now reports every session's footprint under `~/.joy` — media, record, queue, receipts, ledger rows — with size and age, and the relay reports each session's event rows and bytes. The page lists them biggest first across all your machines and says whether each agent is running, detached (process gone) or long gone with only files left; pick any number and delete them in one press. A running session is killed first, and the relay row goes with its events. Loose tmux servers — ones nothing owns any more, live with whatever is still inside them, or just a stale socket file — are listed per machine too and can be killed the same way. Shared files (the ledger, the usage cache) and orphaned files nobody names any more are reported but never touched. Needs an updated daemon on the machine.
...
- **Green means unread, grey means seen.** A session that was online and idle showed the same green whether or not you had read its last reply, so pinned rows sorted "seen" sessions in with the unread ones. Unread is now a state of its own: green is only ever unread, an idle session you have already read is grey, and the pinned order and the collapsed-section dot read top to bottom as amber (needs you) → unread → working → seen → error → offline.
- **Codex starts in yolo.** A new Codex session used to land in Codex's collaborative mode — sandboxed to the workspace, approvals on request, no network — even though the agent defaults said yolo, so Codex would report it "can't change those permissions" and stall. Every harness now defaults to its own no-prompts mode; the mode picker still lets you dial one down. Needs the updated daemon.
- **`/model opus` just switches.** Claude Code sometimes answers a model change with its own "Switch model?" confirm (when the conversation is cached for the old model), and `/effort high` with a slider, and the session sat there until you pressed Enter in the terminal. The daemon now presses it for you, as long as the highlighted answer is Yes. Needs the updated daemon.
