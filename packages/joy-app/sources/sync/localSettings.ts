...
export const LocalSettingsSchema = z.object({
    ...
    markdownCopyV2: z.boolean().describe('Long-press opens the text-selection screen. ON by default: RN Text `selectable` gives NO partial selection on iOS (long press only offers Copy-whole-paragraph), so the TextInput on that screen is the only way to select a phrase.'),
