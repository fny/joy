...
export const LocalSettingsSchema = z.object({
    ...
    nativeTextSelection: z.boolean().describe('iOS: render agent text in a UITextView so a phrase can be selected in place. Off falls back to RN Text, whose iOS long-press only offers copy-the-whole-block. Defaults OFF: it needs a native build that carries the view, and until the probe in modules/joy-selectable-text existed a binary without it rendered every message blank.'),
    markdownCopyV2: z.boolean().describe('Long-press opens the text-selection screen instead of selecting in place. No effect on iOS, which selects in place natively (modules/joy-selectable-text).'),
    consoleLoggingEnabled: z.boolean().describe('Enable console output in production builds'),
    showThinking: z.boolean().describe('Show the agent\'s extended-thinking blocks as collapsed rows in the chat; off hides them entirely'),
