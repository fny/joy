...
export function loadPaletteState(): { themePalette: string; themePaletteDark: string; customPalette: Record<string, string> | null; accentOverrides: Record<string, string> | null; fontOverride: string | null; monoOverride: string | null } {
    const localSettings = mmkv.getString('local-settings');
    if (localSettings) {
        try {
            const parsed = localSettingsParse(JSON.parse(localSettings));
            return { themePalette: parsed.themePalette, themePaletteDark: parsed.themePaletteDark, customPalette: parsed.customPalette, accentOverrides: parsed.accentOverrides, fontOverride: parsed.fontOverride, monoOverride: parsed.monoOverride };
        } catch (e) {
            console.error('Failed to parse local settings for palette', e);
        }
    }
    return { themePalette: localSettingsDefaults.themePalette, themePaletteDark: localSettingsDefaults.themePaletteDark, customPalette: localSettingsDefaults.customPalette, accentOverrides: localSettingsDefaults.accentOverrides, fontOverride: localSettingsDefaults.fontOverride, monoOverride: localSettingsDefaults.monoOverride };
}
