...
export const en = {
    ...
    files: {
        ...
        statusFailed: 'Couldn\'t load changes',
        statusStale: 'Couldn\'t refresh changes — showing the last known state',
        diffStale: 'Couldn\'t refresh this diff — showing the last known version',
    ...
    voice: {
        ...
        refusedTitle: 'Voice call refused',
        refusedMessage: ({ reason }: { reason: string }) => `The agent ended the call right after it connected, before anything was said. ${reason}`,
        refusedOverrides: 'Standby mode overrides the system prompt and first message, which this agent does not allow. Enable both under Security on the ElevenLabs dashboard, or switch Voice to "Stays on".',
        refusedNoReason: 'Check the agent on the ElevenLabs dashboard.',
        end: 'End voice',
    },
    settingsModels: {
        machine: 'Machine',
        machineFooter: 'Catalogs are read from this machine; the choice below applies to every machine.',
        noMachines: 'No machines yet',
        offline: 'offline',
        noPicker: 'This agent has no model picker on this machine.',
        loading: 'Loading catalog…',
        error: ({ error }: { error: string }) => `Could not read the catalog: ${error}`,
        unavailable: 'Machine unreachable',
        empty: 'No models listed',
        recommendedOnly: 'Recommended only',
        recommendedOnlySubtitle: 'Show just the daemon\'s short list (the default)',
        enableAll: 'Enable all',
        enabledCount: ({ enabled, total }: { enabled: number; total: number }) => `${enabled} of ${total} enabled`,
    ...
    settingsVoice: {
        ...
        modeClassic: 'Stays on',
        modeClassicSubtitle: 'One conversation from the mic tap until you end it. Sends nothing the agent has to allow, so a stock agent works as is.',
        modeStandby: 'Standby',
        modeStandbySubtitle: 'Hangs up after silence and wakes for session events or your voice. Needs the system-prompt and first-message overrides enabled on the agent (Security tab).',
        ...
        setupFooter: 'On the ElevenLabs dashboard the agent needs the client tools sendMessageToSession(sessionId, message) and processPermissionRequest(requestId, decision), and optionally the built-in skip_turn tool. Standby mode additionally needs overrides enabled for system prompt and first message (Security tab); "Stays on" needs none.',
        setupPrompt: 'Suggested system prompt',
        setupPromptSubtitle: 'Tap to copy and paste into the agent\'s prompt on the dashboard. In "Stays on" mode the same notes are also sent as context on every connect.',
    },
    machine: {
