...
export const zhHans: TranslationStructure = {
    ...
    files: {
        ...
        statusFailed: '无法加载更改',
        statusStale: '无法刷新更改 — 显示最后已知状态',
        diffStale: '无法刷新此差异 — 显示最后已知版本',
        searching: '正在搜索文件...',
        noFilesFound: '未找到文件',
    ...
    voice: {
        ...
        tapToEnd: '点按结束',
        tapToRetry: '点按重试',
        refusedTitle: '语音通话被拒绝',
        refusedMessage: ({ reason }: { reason: string }) => `代理在连接后立即结束了通话，双方尚未说话。${reason}`,
        refusedOverrides: '“待机”模式会覆盖系统提示词和首条消息，而此代理不允许。请在 ElevenLabs 控制台的 Security 中启用这两项，或将语音切换为“持续在线”。',
        refusedNoReason: '请在 ElevenLabs 控制台中检查该代理。',
        end: '结束语音',
    },
    settingsModels: {
        machine: '机器',
        machineFooter: '目录从这台机器读取；下方的选择适用于所有机器。',
        noMachines: '还没有机器',
        offline: '离线',
        noPicker: '此代理在这台机器上没有模型选择器。',
        loading: '正在加载目录…',
        error: ({ error }: { error: string }) => `无法读取目录：${error}`,
        unavailable: '无法连接机器',
        empty: '没有模型',
        recommendedOnly: '仅推荐',
        recommendedOnlySubtitle: '只显示守护进程的短列表（默认）',
        enableAll: '全部启用',
        enabledCount: ({ enabled, total }: { enabled: number; total: number }) => `已启用 ${enabled} / ${total}`,
    ...
    settingsVoice: {
        ...
        modeTitle: '通话',
        modeClassic: '持续在线',
        modeClassicSubtitle: '从点按麦克风到你结束为止，只有一段对话。不会发送任何需要代理授权的内容，因此标准代理可直接使用。',
        modeStandby: '待机',
        modeStandbySubtitle: '静音后挂断，并在会话事件或你的声音出现时唤醒。需要代理允许覆盖系统提示词和首条消息（Security 标签页）。',
        behaviorTitle: '待命',
        ...
        setupTitle: '代理设置',
        setupFooter: '在 ElevenLabs 控制台中，代理需要客户端工具 sendMessageToSession(sessionId, message) 和 processPermissionRequest(requestId, decision)，可选内置的 skip_turn 工具。“待机”模式还需要启用系统提示词和首条消息的覆盖（Security 标签页）；“持续在线”无需任何设置。',
        setupPrompt: '建议的系统提示词',
        setupPromptSubtitle: '点按复制，然后粘贴到控制台中代理的提示词里。在“持续在线”模式下，同样的说明也会在每次连接时作为上下文发送。',
    },
    machine: {
        environment: '环境变量',
    ...
    message: {
        ...
        unknownTime: '未知时间',
        unopenableGap: ({ count }: { count: number }) => `无法解密 ${count} 条消息 — 密钥更改后将重试`,
    },
    ...
    markdown: {
        ...
        loadImage: ({ host }: { host: string }) => `从 ${host} 加载图片`,
        // A task notification sharing a message with a real prompt renders as a line (#269)
        taskNotificationLine: ({ status, summary }: { status: string; summary: string }) => `后台任务 ${status}：${summary}`,
    },
