...
export const zhHant: TranslationStructure = {
    ...
    settingsFeatures: {
        ...
        nativeTextSelection: '就地選取文字',
        nativeTextSelectionSubtitle: 'iOS：長按訊息以選取詞句',
    ...
    sidebar: {
        ...
        showAutomations: '顯示自動化',
        hideAutomations: '隱藏自動化',
        showHeadless: '顯示無人值守',
        hideHeadless: '隱藏無人值守',
        sortByProject: '按專案',
        sortByState: '按狀態',
    ...
    voice: {
        ...
        tapToEnd: '點按結束',
        tapToRetry: '點按重試',
        refusedTitle: '語音通話被拒絕',
        refusedMessage: ({ reason }: { reason: string }) => `代理在連線後立即結束了通話，雙方尚未說話。${reason}`,
        refusedOverrides: '「待機」模式會覆寫系統提示詞和第一則訊息，而此代理不允許。請在 ElevenLabs 控制台的 Security 中啟用這兩項，或將語音切換為「持續連線」。',
        refusedNoReason: '請在 ElevenLabs 控制台中檢查該代理。',
        end: '結束語音',
    },
    settingsModels: {
        machine: '機器',
        machineFooter: '目錄從這台機器讀取；下方的選擇適用於所有機器。',
        noMachines: '還沒有機器',
        offline: '離線',
        noPicker: '此代理在這台機器上沒有模型選擇器。',
        loading: '正在載入目錄…',
        error: ({ error }: { error: string }) => `無法讀取目錄：${error}`,
        unavailable: '無法連線到機器',
        empty: '沒有模型',
        recommendedOnly: '僅推薦',
        recommendedOnlySubtitle: '只顯示守護程式的短清單（預設）',
        enableAll: '全部啟用',
        enabledCount: ({ enabled, total }: { enabled: number; total: number }) => `已啟用 ${enabled} / ${total}`,
        filterPlaceholder: '篩選模型',
        default: '預設',
        recommended: '推薦',
    },
    settingsVoice: {
        ...
        modeTitle: '通話',
        modeClassic: '持續連線',
        modeClassicSubtitle: '從點按麥克風到你結束為止，只有一段對話。不會傳送任何需要代理授權的內容，因此標準代理可直接使用。',
        modeStandby: '待機',
        modeStandbySubtitle: '靜音後掛斷，並在工作階段事件或你的聲音出現時喚醒。需要代理允許覆寫系統提示詞和第一則訊息（Security 分頁）。',
        behaviorTitle: '待命',
        ...
        setupTitle: '代理設定',
        setupFooter: '在 ElevenLabs 控制台中，代理需要用戶端工具 sendMessageToSession(sessionId, message) 和 processPermissionRequest(requestId, decision)，可選內建的 skip_turn 工具。「待機」模式還需要啟用系統提示詞和第一則訊息的覆寫（Security 分頁）；「持續連線」不需要任何設定。',
        setupPrompt: '建議的系統提示詞',
        setupPromptSubtitle: '點按複製，然後貼到控制台中代理的提示詞裡。在「持續連線」模式下，同樣的說明也會在每次連線時作為情境傳送。',
    },
    machine: {
        environment: '環境變數',
