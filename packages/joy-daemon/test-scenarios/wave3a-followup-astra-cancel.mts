...
const root=process.env.JOY_REVIEW_SRC ?? '/tmp/joy-test-tmux/review3/wave3a-followup-astra-checkout/packages/joy-daemon/src';
...
const s:any={id:'local',cwd:dir,status:'active',cardMetadata:()=>null,setV2Link(){},busy:()=>busy,queueState:()=>({pendingCount:0,paused:false}),queueItemState:()=> 'delivered',enqueue(){enqueues++;busy=true;return {id:'queue-item',...(scenario.startsWith('handled_')?{handled:'command',...(scenario==='handled_start'?{reinjectionId:'rein-item'}:{})}:{})}},cancelQueued(){plucks++;return false},async abort(){aborts++;busy=false}};
...
globalThis.fetch=(async(input:any,init:any={})=>{
 ...
 if(path.endsWith('/start')&&(scenario==='start_rejected'||scenario.startsWith('handled_')))return Response.json({error:'turn_cancelled'},{status:409});
...
try{
 ...
 }else if(scenario.startsWith('handled_')){
  await until(()=>!!terminal);assert.equal(enqueues,1);assert.equal(aborts,1);assert.equal(plucks,scenario==='handled_start'?1:0);assert.equal(busy,false);assert.equal(terminal.terminalState,'cancelled');console.log('handled-command start rejected: adapter aborted (even for handled_title with no reinjection), cancelled terminal');
 }else{
