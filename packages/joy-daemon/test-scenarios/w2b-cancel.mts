...
const s:any={id:'local',cwd:dir,status:'active',cardMetadata:()=>null,setV2Link(){},busy:()=>busy,queueState:()=>({pendingCount:0,paused:false}),queueItemState:()=> 'delivered',enqueue(){enqueues++;busy=true;return {id:'queue-item'}},cancelQueued(){plucks++;return true},async abort(){aborts++;busy=false}};
...
globalThis.fetch=(async(input:any,init:any={})=>{
 ...
 if(path.includes('/claims/')){
  await sleep(10);
  if(path.endsWith('/work')&&!offered){offered=true;return Response.json({offers:[{kind:'prompt',sessionId:row.sessionId,turnId:'turn',commandId:'cmd',deliveryId:'prompt-delivery',attachments:scenario==='download'?[{id:'attachment',size:4}]:[],ciphertext:JSON.stringify({v:1,t:'plain',text:'test',attachments:scenario==='download'?[{id:'attachment',name:'photo.txt',size:4}]:[]})}]})}
 ...
 if(path.startsWith('/attachments/')){downloading=true;await download;return new Response('data')}
 if(path.endsWith('/start')&&scenario==='start_rejected')return Response.json({error:'turn_cancelled'},{status:409});
...
try{
 if(scenario==='download'){
  ...
  const files=readdirSync(dir).filter(f=>f.includes('photo'));assert.equal(files.length,1);
  console.log('cancel during attachment download: no enqueue; cancelled terminal; materialized file left behind',JSON.stringify(files));
 }else{
  await until(()=>!!terminal);assert.equal(enqueues,1);assert.equal(aborts,0);assert.equal(plucks,0);assert.equal(busy,true);
  console.log('start rejected as turn_cancelled:',JSON.stringify({enqueues,aborts,plucks,busy,terminal:terminal.terminalState}));
 }
 console.log('PASS: actual lane with controlled relay timing and adapter boundary.');
