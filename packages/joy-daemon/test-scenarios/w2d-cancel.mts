...
const s:any={id:'local',cwd:dir,status:'active',cardMetadata:()=>null,setV2Link(){},busy:()=>busy,queueState:()=>({pendingCount:0,paused:false}),queueItemState:()=> 'delivered',enqueue(){enqueues++;busy=true;return {id:'queue-item',...(scenario==='handled_start'?{handled:'command'}:{})}},cancelQueued(){plucks++;return true},async abort(){aborts++;busy=false}};
...
try{
 if(scenario==='download'){
  ...
  await until(()=>!!terminal);assert.equal(enqueues,1);assert.equal(aborts,0);assert.equal(plucks,0);assert.equal(busy,true);assert.equal(terminal.terminalState,'failed');console.log('handled-command start rejected: local work still busy, zero aborts, failed terminal');
 }else{
  ...
  assert.equal(readdirSync(dir).filter(f=>f.includes('photo')).length,1);console.log('start rejected: agent aborted, but attachment remains');
