...
const root='/tmp/joy-test-tmux/review3/wave3b-daemon-astra-checkout/packages/joy-daemon/src';
...
assert.equal(existsSync(joyStateDir()+'/codex-inbound-'+s.id+'.json'),false);assert.equal(attachWrites,0);
console.log('late thread/start after kill still recreates window record, but no inbound file or attach (outer fence is too late)');
