...
import {root,codex,opencode,relay} from './wave2e-lifecycle-astra-helpers.mts';
...
}
{
 ...
 const users=rs.rows.filter((r:any)=>r.record.role==='user');assert.equal(users.length,2);assert.notEqual(users[0].localId,users[1].localId);
 console.log('own RPC already removed from spool before crash is replayed as external; duplicate user ids',users.map((u:any)=>u.localId));
}
