...
try{
 ...
 assert.ok(Date.now()-start<1900);assert.equal(running(serverPid),true);
 console.log('one unrelated /proc ENOENT makes group kill return while TERM-ignoring server child survives');
