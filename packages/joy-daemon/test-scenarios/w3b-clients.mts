...
wss.on('connection',(ws:any)=>{
 ...
 ws.on('message',(buf:any)=>{
  ...
  if(msg.method==='initialize'){
   if(attempt===4){ws.send(JSON.stringify({jsonrpc:'2.0',id:902,method:'approval',params:{}}));setTimeout(()=>ws.send(JSON.stringify({jsonrpc:'2.0',id:msg.id,result:{ok:true}})),60);return}
...
answerNew({approved:false});await sleep(50);assert.equal(replies.length,0);
console.log('replacement external-resolution marker survived old handler; no duplicate reply');
// A suppression marker from the old generation must not suppress a new request.
let old901!:(v:any)=>void,new901!:(v:any)=>void;
client.onServerRequest(()=>new Promise(r=>old901=r));
sockets[1].send(JSON.stringify({jsonrpc:'2.0',id:901,method:'approval',params:{}}));
await until(()=>!!old901);client.resolveServerRequestExternally(901);
await client.connect(dir+'/app.sock',500);sockets[1].terminate();
old901({approved:true});await sleep(20);
client.onServerRequest(()=>new Promise(r=>new901=r));
sockets[2].send(JSON.stringify({jsonrpc:'2.0',id:901,method:'approval',params:{}}));await until(()=>!!new901);
let observed901=false;sockets[2].on('message',(b:any)=>{if(JSON.parse(b.toString()).id===901)observed901=true});
new901({approved:true});await sleep(50);assert.equal(observed901,true);
console.log('old external-resolution marker is cleared on reconnect; legitimate reused-id reply succeeds');
let answer902!:(v:any)=>void;client.onServerRequest(()=>new Promise(r=>answer902=r));const connecting=client.connect(dir+'/app.sock',500);await until(()=>!!answer902);client.resolveServerRequestExternally(902);await connecting;let replied902=false;sockets[3].on('message',(b:any)=>{if(JSON.parse(b.toString()).id===902)replied902=true});answer902({approved:true});await sleep(30);assert.equal(replied902,true);console.log('REGRESSION new-generation external-resolution marker set during initialize is cleared at handshake completion, so a duplicate reply is sent');
