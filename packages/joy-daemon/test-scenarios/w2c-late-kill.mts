...
s.end('process_exited');s.forceKill();
assert.equal(loadWindowRecord(s.id),null);
console.log('ended forceKill: record deleted, old startup still awaiting model reply');
unlock();
for(let i=0;i<100&&!loadWindowRecord(s.id);i++)await new Promise(r=>setTimeout(r,5));
assert.equal(s.status,'ended');assert.equal(loadWindowRecord(s.id),null);
console.log('late old startup: record remains deleted after ended forceKill',JSON.stringify({status:s.status,reason:s.endReason,record:!!loadWindowRecord(s.id)}));
mkdirSync(cwd,{recursive:true});
const next=new SessionRegistry({tmuxSession:'test',relayClient:null});next.recover();
assert.equal(next.get(s.id),undefined);assert.equal(spawned,1);
console.log('next recover: killed session not resurrected');
