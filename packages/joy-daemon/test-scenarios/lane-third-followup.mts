...
process.env.JOY_HOME_DIR=mkdtempSync('/tmp/joy-test-tmux/review3/wave1-astra-third-followup-');
const root='../src';
...
const fastSweep=['duplicates','replay_retry','sweep','claim_race','fence','sweep_health','late_sink','handoff','dispatch'].includes(scenario);
if(fastSweep)globalThis.setTimeout=((fn:any,ms:any,...args:any[])=>realSetTimeout(fn,ms===8000?30:ms===1000?20:ms,...args)) as any;
const row={sessionId:'session-one',localSessionId:'local-one',daemonId:'machine-one',state:'active',queuedTurns:1,executing:scenario==='dispatch'?null:'running'};
...
if(scenario==='cap'||scenario==='growth'||scenario==='dispatch')initial=Array.from({length:2000},(_,i)=>out('old-'+i));
if(scenario==='dispatch')initial.push(out('old-2000'));
...
let busy=false,offered=false,claimReady=false,claimFinished=false;const enqueued:string[]=[];
const session:any={id:row.localSessionId,status:'active',cardMetadata:()=>null,busy:()=>busy,queueState:()=>({pendingCount:0,paused:false}),queueItemState:()=> 'delivered',abort:async()=>{busy=false;},enqueue(text:string){enqueued.push(text);
...
globalThis.fetch=async(input:any,init:any={})=>{
 ...
 if(path.includes('/claims/')){
  await sleep(10);
  if((['live_terminal','quota','claim_race','handoff'].includes(scenario)||(scenario==='dispatch'&&!claimFinished))&&path.endsWith('/work')&&(!offered||scenario==='dispatch')&&(scenario!=='claim_race'||claimReady)){
 ...
 if(path.endsWith('/facts')&&body.type==='terminal'){claimFinished=true;if(scenario==='handoff'){timeShift=61000;return response({error:'temporary'},503);}events.push('terminal:'+body.terminalState);terminalState??=body.terminalState;return response();}
 if(path.endsWith('/facts')&&body.type==='output'){
  events.push('request:'+body.runtimeEventId);
  if(['duplicates','live_terminal','cap','growth','dispatch','disk_receipt','disk_opencode','sweep_health','late_sink'].includes(scenario))await outputGate;
...
try{
 if(scenario==='boot_order'){
  ...
  await until(()=>logs.some(s=>s.includes('dispatch paused until they drain')));
  assert.equal(enqueued.length,0);assert.equal(calls.filter(c=>c.path.endsWith('/received')).length,0);
  assert.equal(pending().size,2001);
  openOutput();await until(()=>pending().size===0);
  timeShift=16000;await until(()=>terminalState==='completed');
  assert.deepEqual(enqueued,['/title hi']);
  assert.equal(calls.filter(c=>c.path.endsWith('/received')).length,1);
  console.log(JSON.stringify({scenario,result:'fixed',blockedWhileOverCap:true,receivedAfterDrain:1,enqueued}));
  ...
  if(scenario==='sweep_health'||scenario==='late_sink'){
   if(scenario==='late_sink')codex.attachRelay(adapter);
   const before=reads;await until(()=>reads>=before+4);
   assert.equal(adapter.outboundPersistDegraded,true);
   assert.equal(loadCheckpoint(row.localSessionId).deliveredThroughTurnId,null);
   assert.equal(pending().size,0);
   // Repair the disk only; keep the output POST blocked. The sweep must really resave.
  ...
  assert.equal(pending().size,scenario==='disk_receipt'?2:1);
  console.log(JSON.stringify({scenario,result:scenario==='disk_receipt'?'held until actual add save':'held across failed sweeps, released after resave',degraded:adapter.outboundPersistDegraded,deliveredThrough:loadCheckpoint(row.localSessionId).deliveredThroughTurnId,spooled:pending().size,outputAcks:events.filter(x=>x.startsWith('ack:')).length}));
