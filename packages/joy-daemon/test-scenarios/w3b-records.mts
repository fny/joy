...
process.env.JOY_REVIEW_SRC='/tmp/joy-test-tmux/review3/wave3b-daemon-astra-checkout/packages/joy-daemon/src';
const {root,codex,opencode,relay}=await import('./wave2d-astra-helpers.mts');
...
}
{
 ...
 console.log('ordinary recovery history: missed TUI prompt and its answer both replayed');
}
{
 ...
 console.log('recovery confirms/removes own RPC spool item; buffered echo retains ownership and is not mirrored');
}
{
 // A Joy RPC echo was already confirmed before the daemon crashed, so the
 // pending spool no longer contains its UUID when recovery begins.
 const id='c0dec215',rs=relay(),before=codex({id});before.attachRelay(rs as any);
 before.__setup({turnStart:async()=>({turnId:'unfinished'})});
 const q=before.enqueue('already mirrored Joy RPC');
 const item={id:'user',type:'userMessage',clientId:q.id,text:'already mirrored Joy RPC'};
 before.__event({method:'item/completed',params:{threadId:'thread',turnId:'unfinished',item}});
 assert.equal(before.queueItemState(q.id),'delivered');
 const after=codex({id,codexThreadId:'thread'});after.attachRelay(rs as any);after.__setup({});
 await after.__replay({threadRead:async()=>({thread:{turns:[{id:'unfinished',status:'inProgress',items:[item]}]}})});
 const users=rs.rows.filter((r:any)=>r.record.role==='user');assert.equal(users.length,1);
 console.log('own RPC already removed from spool before crash retains checkpoint ownership; one user id',users.map((u:any)=>u.localId));
}
