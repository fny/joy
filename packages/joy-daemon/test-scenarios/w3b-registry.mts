...
// A joiner can reserve the very id the pending restart needs to construct.
{
 ...
 assert.ok(out.every((o:any)=>o.state==='resolved'));assert.equal(out[0].v,out[1].v);assert.equal(r.get(s.id),out[0].v);
 console.log('create joining retiring restart: both join the same successful replacement');
}
{
 ...
 for(const identify of [{continue:true},{resume_id:'conversation'}]){
  ...
  for(const extra of [{fallbackModel:'other'},{chrome:true},{extraArgs:'--different-setting'}])await assert.rejects(r.create({cwd:dir,...identify,...extra}),/different settings/);
 }
 console.log('resume/continue compare model correctly, and reject differing fallbackModel/chrome/extraArgs');
 s.detectPermissionMode=()=> 'plan';assert.equal(await r.create({cwd:dir,continue:true,permissionMode:'plan',yolo:true}),s);
 console.log('matching explicit plan mode with overridden yolo:true correctly adopts matching live mode');
 assert.notEqual(await r.create({cwd:dir,continue:true,forceNew:true,model:'sonnet'}),s);
 console.log('forceNew + continue creates a new session');
}
