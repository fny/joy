...
const entries=await call('/v2/sessions/repo/git/entries?untracked=1');assert.equal(entries.data.ok,true);assert.ok(entries.data.files.includes('untracked.txt'));assert.ok(!entries.data.files.includes('ignore.me'));assert.ok(!entries.data.files.includes('quo"te.txt'));console.log('entries quote/newline names:',entries.data.files.filter((s:string)=>s.includes('quo')||s.includes('lines')));
const wild=await call('/v2/sessions/repo/git/diff?path='+encodeURIComponent('a[1].txt'));assert.equal(wild.data.ok,true);console.log('bracket path diff:',JSON.stringify(wild.data.diff));
...
const unborn=await call('/v2/sessions/fresh/git/diff?head=1&path=first.txt');assert.equal(unborn.data.ok,false);console.log('unborn HEAD:',unborn.data.error.split('\n')[0]);
...
console.log('PASS actual HTTP routes: untracked/ignored, HEAD, abort. Reproduced quoted entries, pathspec expansion, unborn HEAD.');process.exit(0);
