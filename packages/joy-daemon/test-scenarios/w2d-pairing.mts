...
assert.deepEqual(readEnvStore(),{ok:true,env:{TEST_PROVIDER_TOKEN:'test-value'}});
console.log('divergent sibling keys: third relay selected the key that opens the store');
await pairWithRelay('https://aaa-legacy.test',secret,legacy);process.env.JOY_RELAY_URL='https://aaa-legacy.test';paths.__resetRelaySelection();assert.deepEqual(readEnvStore(),{ok:true,env:{TEST_PROVIDER_TOKEN:'test-value'}});console.log('re-pair divergent destination: wrong old key replaced by sibling key that opens store');
