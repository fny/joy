...
const fastSweep=['duplicates','replay_retry','sweep','claim_race','fence'].includes(scenario);
...
globalThis.fetch=async(input:any,init:any={})=>{
 ...
 if(path.includes('/claims/')){
  await sleep(10);
  if(['live_terminal','quota','claim_race'].includes(scenario)&&path.endsWith('/work')&&!offered&&(scenario!=='claim_race'||claimReady)){
 ...
 if(path.endsWith('/facts')&&body.type==='terminal'){events.push('terminal:'+body.terminalState);terminalState??=body.terminalState;return response();}
 if(path.endsWith('/facts')&&body.type==='output'){
  events.push('request:'+body.runtimeEventId);
  if(['duplicates','live_terminal','cap','disk_receipt','disk_opencode'].includes(scenario))await outputGate;
...
try{
 if(scenario==='boot_order'){
  ...
  assert.deepEqual(events.slice(0,5),['request:rec:old-first','ack:rec:old-first','request:rec:new-second','ack:rec:new-second','terminal:completed']);
  console.log(JSON.stringify({scenario,result:'initial order and saved outcome fixed; duplicate scheduling remains',events}));
  ...
  await until(()=>events.includes('ack:rec:new-live'));
  openStatus();await until(()=>events.includes('ack:rec:old-first'));
  assert.equal(events[0],'request:rec:new-live');
  console.log(JSON.stringify({scenario,result:'defect reproduced',events}));
 }else if(scenario==='duplicates'){
  await until(()=>reads>=6);openOutput();await until(()=>events.filter(x=>x==='ack:rec:second').length>=3);
  console.log(JSON.stringify({scenario,result:'defect reproduced',reads,firstPosts:events.filter(x=>x==='request:rec:first').length,secondPosts:events.filter(x=>x==='request:rec:second').length,spoolSize:pending().size}));
 }else if(scenario==='live_terminal'){
  await until(()=>calls.some(c=>c.path.endsWith('/start')));await sleep(100);
  assert.equal(pending().hasTerminalFor(turnId),false);
  console.log(JSON.stringify({scenario,result:'defect reproduced',pendingKinds:pending().all().map(e=>e.kind),terminalState}));
  ...
  const disk=pending();assert.equal(disk.size,2000);assert.ok(!disk.all().some(e=>e.id==='old-0'));assert.equal(adapter.outboundPersistDegraded,false);
