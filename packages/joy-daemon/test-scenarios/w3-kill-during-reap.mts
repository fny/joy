...
(globalThis as any).__mock={OpencodeClient:Client,isOpencodeServerPid:()=>true,killOpencodeServerPid:async()=>{reaping=true;await gate;return true},spawnOpencodeServer(){spawned++;return {proc:new EventEmitter(),port:Promise.resolve(1234)}}};
...
release();await new Promise(r=>setTimeout(r,40));assert.equal(spawned,0);assert.equal(created,0);assert.equal(s.end('killed'),false);
console.log('kill during awaited reaping: ended session starts no replacement after forceKill; repeated end is a no-op');
