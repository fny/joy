...
for(const [name,Ctor] of [['claude',Session],['pi',PiSession],['codex',CodexSession],['opencode',OpencodeSession],['agy',AgySession]] as const){
 ...
 assert.equal(!!loadWindowRecord(id),name!=='claude');
}
for(const op of [kill,killSession]){
 ...
 assert.deepEqual(out,op===kill?{ok:true}:{success:false,error:'archive failed'});
}
...
for(const [name,pane] of [['running',running],['dialog',dialog],['trust',trust]]){
 ...
 assert.equal(s.status,name==='trust'?'active':'ended');
}
console.log('PASS: lifecycle diagnostics reproduced with actual classes and operations; poll uses copied class plus test hook.');
process.exit(0);
