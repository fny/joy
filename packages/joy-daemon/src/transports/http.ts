 ...
 *  item but the first (Astra on 81386fd0). Items are ENCODED into byte
 ...
 *  stalled response awaited drain (Astra on 2615f870) — now no more than
 ...
 *  by the 8 MiB cap (Astra on 52d6c86d). The store is now PAGED — this many
...
function readJsonBody(req: IncomingMessage): Promise<unknown> {
  ...
  return new Promise(resolve => {
    ...
    // U+FFFD (#62 family; Astra on 4b4974fa).
    const parts: Buffer[] = [];
    let size = 0;
    let overflow = false;
...
<script>Redoc.init(${specJs}, {}, document.getElementById("redoc"));</script>
    ...
    if (evm) {
      ...
      // (#597; Astra on 4b4974fa and Wave B) — see streamHistoryThenFollow.
