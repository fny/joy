...
export class CodexSession implements AgentSession {
  ...
  #onDispatchEchoed(clientId: string): void {
    ...
    if (this.#inbound.length !== before) {
      ...
      // durable copy of "this prompt was ours" somewhere (Astra on 4cff51bf).
      ...
      if (!saved) {
        ...
        // to that check — a second entry, a second turn (Astra on 8457d44e).
  ...
  #dispatchNotification(n: CodexNotification): void {
    ...
    if (n.method === "turn/started") {
      ...
      // unrelated TUI turn (Astra on e573e1a9).
  ...
  queueItemState(id: string): "pending" | "delivered" | "cancelled" | "failed" | "unknown" {
    ...
    // pending held the lane until its dispatch cap (Astra on e2f052c5, #78).
    ...
    // no_agent_activity (Astra on 975eef1d, #516). The receipt IS the outcome.
