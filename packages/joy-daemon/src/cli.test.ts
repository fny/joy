...
describe("verifyDaemonPid (#495)", () => {
  ...
  test("serverEntryOf: the script is the first operand after node's options — its ROLE, not its occurrence", () => {
    ...
    // our path as a trailing ARGUMENT to another program (Astra on df7dc0a0)
  ...
  test("serverEntryOf: node -e / -p / stdin / REPL run inline code, not a script — our path after them is that program's data (Astra on e9d934ef)", () => {
  ...
  test("serverEntryOf: OS argv boundaries survive — a loader path with a space is one argument (Astra on e9d934ef)", () => {
