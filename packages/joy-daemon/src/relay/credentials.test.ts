...
describe('loadCredentials', () => {
    ...
    it('falls back to ~/.happy/access.key when joy-native is absent', () => {
        ...
        writeFileSync(join(happyHome, 'access.key'), JSON.stringify({ token: 'happy-token' }));
    ...
    it('joy-native credentials.json overrides the .happy fallback', () => {
        ...
        writeFileSync(join(happyHome, 'access.key'), JSON.stringify({ token: 'happy-token' }));
        writeFileSync(join(joyHome, 'credentials.json'), JSON.stringify({ token: 'joy-token', serverUrl: 'https://from.joy' }));
        const c = loadCredentials();
    ...
    it('JOY_CREDS env wins over JOY_HOME/credentials.json and .happy', () => {
        const { joyHome, happyHome } = mkSandbox();
        writeFileSync(join(happyHome, 'access.key'), JSON.stringify({ token: 'happy-token' }));
        writeFileSync(join(joyHome, 'credentials.json'), JSON.stringify({ token: 'joy-token' }));
        const explicit = join(joyHome, 'explicit.json');
        writeFileSync(explicit, JSON.stringify({ token: 'env-token', serverUrl: 'https://explicit' }));
