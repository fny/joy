...
export function startNucleusLane(opts: NucleusLaneOpts): NucleusLaneHandle {
  ...
  const writeSpawnIntent = (commandId: string, localId: string): void => {
    ...
    // mapping (Astra on 57fb90a3, #75); Wave B routes the remaining hand-rolled
   ...
   *  completion rate with no pause (Astra on 2ff0e215, #8). */
  ...
  // small backlog is not evidence of persistence (Astra, afb8ac84).
   ...
   *  for an entry that is waiting behind a blocked head (Astra, dee28922). */
