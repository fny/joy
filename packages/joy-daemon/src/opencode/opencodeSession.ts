...
export class OpencodeSession implements AgentSession {
  ...
  async #drainInboundInner(client: OpencodeClient, ocSessionId: string): Promise<void> {
    for (const item of [...this.#inbound]) {
      ...
      try {
        ...
        // dropped stream never delivers (Astra on e2f052c5, #77).
   ...
   *  cancelled (Astra on 8457d44e, #77). A kept tombstone lets the next
   ...
   *  an interrupt at newer work (Astra on 0b9db1f5, #77). */
  #noteAdmitted(id: string, serverSeq?: number): void {
    ...
    // rank, became newest and interrupted B (Astra on 2263a3ca, #77).
    ...
    // session-wide interrupt at current work (Astra on a0e0ee66). Evict only
  ...
  #interruptCancelled(id: string, client: OpencodeClient, ocSessionId: string): void {
    ...
    // f307c11b). Later evidence retries only after a failure settled.
    ...
    void client.interrupt(ocSessionId)
      ...
      .catch((e) => {
        ...
        // retry: the tombstone alone is the evidence (Astra on 7dd135c6, #77).
        ...
        // instead (Astra on 9edf80fb). The tombstone then stays for the next
        // admission evidence of THIS prompt, if any.
        // One pending retry timer per id: an HTTP failure and a later SSE
        // failure used to arm two (Astra on 0b9db1f5).
