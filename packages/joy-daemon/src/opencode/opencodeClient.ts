...
export function spawnOpencodeServer(cwd: string, opts?: { bin?: string; joySessionId?: string }): OpencodeSpawnResult {
  ...
  const port = new Promise<number>((resolve, reject) => {
    ...
    let settled = false;
    const timer = setTimeout(() => { if (!settled) { settled = true; reject(new Error("opencode serve: no listen line within 30s")); } }, 30_000);
    const onLine = (line: string) => {
      const m = /listening on http:\/\/127\.0\.0\.1:(\d+)\b/.exec(line);
      if (m && !settled) { settled = true; clearTimeout(timer); proc.stdout?.off("data", onOut); proc.stderr?.off("data", onErr); resolve(parseInt(m[1], 10)); }
    };
    const onOut = (d: Buffer) => { for (const l of out.push(d)) onLine(l); };
    const onErr = (d: Buffer) => { for (const l of err.push(d)) onLine(l); };
    proc.stdout?.on("data", onOut);
    proc.stderr?.on("data", onErr);
    proc.on("exit", (code) => { if (!settled) { settled = true; clearTimeout(timer); reject(new Error(`opencode serve exited during startup (code ${code})`)); } });
...
export async function killOpencodeServerPid(pid: number): Promise<boolean> {
  ...
  const survivors = (): number[] => {
    ...
    for (const d of entries) {
      ...
      // the whole scan (that hid the real survivor; Astra on 8df5fd06).
      try {
