...
// ── cb83f005 review: ingress fence, wire contract, live-process end, subagents, turn authority ──
...
test("late transcript terminal (edd69fd1): the interrupt marker keeps its authority for the OPEN hook turn (no Stop fires on Esc) and loses it for an older one", async () => {
  vi.useFakeTimers();
  const { driver } = fakeTmux({ pane: READY });
...
// ── cb83f005 review residuals: launch identity, fresh turn-start evidence, #480 persistence ──
