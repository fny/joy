...
export class Session {
  ...
  // 32f6ed84). Bounded; an evicted position means a receipt without one,
   ...
   *  showed an empty box while the real one held the unsent text (cb83f005
   ...
   *  clear its confirmation ref (Astra on edd69fd1: Stop A → dispatch B →
  ...
  startTailer(transcriptPath: string, force = false): void {
    ...
    this.#tailer = self = tailJsonl(
      ...
      (entry, pos) => {
        ...
        // by where it actually is (review 32f6ed84). A no-op when unchanged.
  ...
  #pollEnd(): void {
    ...
    if (this.pid !== undefined && !Session.#pidAlive(this.pid)) {
      ...
      if (fresh !== undefined) {
        ...
        // the other and never expired (Astra on 57fb90a3).
  ...
  onTranscriptEntry(entry: Record<string, unknown>): void {
    ...
    if (entryType === "system" && (entry.subtype === "stop_hook_summary" || entry.subtype === "turn_duration")) {
      ...
      // (#transcriptOwnsTerminal; Astra on edd69fd1).
