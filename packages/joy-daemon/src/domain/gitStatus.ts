...
export async function readGitStatus(cwd: string, git: GitRunner = runGit): Promise<GitStatusResult> {
  ...
  // read (ok:false), never a guessed root (review residual on 89c8d5a8, #1).
  ...
  const [status, staged, unstaged, refs] = await Promise.all([
    ...
    // residual on 89c8d5a8, #2). core.quotepath does not need pinning: with
  ...
  // 89c8d5a8, #2: a modified file read "unavailable" while the totals
