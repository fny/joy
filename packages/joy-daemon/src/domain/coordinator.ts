...
interface Actor {
   ...
   *  no second Escape goes out until it settles (Astra on 7b6751c8). */
...
export class SessionCoordinator {
  ...
  async #pumpLoop(actor: Actor): Promise<void> {
    ...
    for (;;) {
      ...
      // verdict, then re-plan from the ledger (Astra on 7b6751c8).
      ...
      if (driver.prepare && !asSteer) {
        ...
        // (Astra on edd69fd1); the head that is still current is dispatched.
   ...
   *  tombstone alike (Astra on edd69fd1: a bare driver.interrupt for late
  ...
  async #interruptOp(actor: Actor, commandId: string): Promise<InterruptResult | { kind: "withheld"; error: string } | null> {
    ...
    for (;;) {
      ...
      // stopping the wrong turn (Astra on 9edf80fb, #77). A second session-wide
   ...
   *  edd69fd1). Evidence may move the command only when the attempt it
