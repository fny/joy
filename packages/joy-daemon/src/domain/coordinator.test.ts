...
test("harness w2e-queue: 'Codex turn/started after timeout alone misses the tombstoned item; the later correlated echo interrupts it'", async () => {
  ...
  d.emit({ kind: "turn_started", runtimeTurnId: "late-own" }); // a turn/started alone is not attributable (Astra on e573e1a9)
  await settle();
  expect(turnInterrupts(d)).toEqual([]);
...
// ── edd69fd1 review residuals: the editable queue across the pre-attempt gate ──

test("pane gate (edd69fd1): an edit while the head waits at prepare re-plans — the driver receives the edited text and the attempt records that payload version", async () => {
...
test("pane gate (edd69fd1): a reorder while the head waits at prepare re-plans — the new head is submitted first", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
  const gate = deferred<"ready" | "cancelled" | "retired">();
  const prepared: string[] = [];
  d.prepare = (cmd) => { prepared.push(cmd.text); return gate.promise; };
...
test("observation fencing (edd69fd1): a timed-out attempt's late echo and turn end are history on THAT attempt — the edited, re-submitted replacement moves only on its own evidence", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
...
test("observation fencing (edd69fd1): a late echo of the pre-edit attempt does not run the edited queued row; without an edit the same late echo still runs it (the timed-out submission landed)", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
...
test("tombstone (edd69fd1): late evidence of a cancelled timed-out prompt never issues an untargeted interrupt while the current command runs — deferred through the scheduler, dropped when the run ends", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
...
test("tombstone (edd69fd1): with nothing else running the late turn is interrupted ONCE through the scheduler; a failed result retries on the cancel budget and is bounded", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
  const old = accept("s1", "old");
  await settle();
  const ref = d.lastSubmit.attempt.runtimeRef;
...
// ── collateral / serialization boundaries (Astra on 7b6751c8) ───────────────
...
test("collateral (7b6751c8): a late echo of a cancelled row while the newer command is still SUBMITTING issues no untargeted interrupt until that submission settles", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
...
test("collateral (7b6751c8): a submission that settles as a permanent rejection frees the parked tombstone interrupt — it goes out then, once", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
...
test("collateral (7b6751c8): an unrelated foreign (TUI) turn is protected — a late echo naming ANOTHER turn parks the interrupt until the foreign turn ends", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
  const { old, ref } = await cancelledTimedOut(d);
...
test("serialization (7b6751c8): a pending untargeted interrupt gates a new accept — driver.prepare / submit run only once the interrupt settled", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
  const { ref } = await cancelledTimedOut(d);
  let prepares = 0;
...
test("serialization (7b6751c8): an untargeted interrupt that starts DURING the pane gate wait is awaited after the gate; the head is re-planned and gated again before it is submitted", async () => {
  const d = session("s1", CLAUDE_LIKE, "claude");
  const { ref } = await cancelledTimedOut(d);
