...
describe("readGitStatus on real repositories", () => {
  ...
  test("a failed --show-prefix in a subdirectory session is a failed read, never a guessed root (89c8d5a8 residual #1)", async () => {
  ...
  test("diff.relative=true: numstat still joins porcelain's root-relative paths (89c8d5a8 residual #2)", async () => {
  ...
  test("a changed text entry that numstat did not cover makes that side's total unavailable, not zero (89c8d5a8 residual #2)", async () => {
