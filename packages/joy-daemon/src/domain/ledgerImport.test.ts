...
// ── review 75e30a4f: malformed ROWS / FIELDS fail the source like a malformed envelope ──
...
test("a window record that does not parse (a truncating write) is a FAILED import of that record: quarantined, not stripped, not done — a sibling still imports; the repair completes it (review 3d615fae)", () => {
  const file = join(dir, "window-abcdef12.json");
...
test("a handoff job resume could not act on is a FAILED import, never stripped: a source without its target, a target without its peer, an unknown role or agent, delivered without dst (review 3d615fae)", () => {
...
// ── review 7679a27e: receipt rows fail closed; a failed window record holds its dependent sources ──
