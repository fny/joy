...
//     the newer attempt (review 05bf2445, wave C1). transition / setCheckpoint
 ...
 *  claimed by a second session (review 75e30a4f). */
 ...
 *  handed the first session's row (review 85abc7d8). */
 ...
 *  only thing a committed cursor can cover (review 32f6ed84). Null = the
...
export interface CheckpointRow {
   ...
   *  the offset a recovery tailer later commits — review 32f6ed84), so a
...
export interface ObservationEffects {
  receipts?: NewReceipt[];
  outbox?: NewOutbound[];
  /** `expectedAttemptId` (optional, additive — Astra on edd69fd1): the
 ...
 *  with no recorded position is never covered (review 32f6ed84: insertion
...
export class Ledger {
   ...
   *  positions (review 32f6ed84): existing receipts keep `transcript_path`
   ...
   *  (review 75e30a4f, 85abc7d8): a caller-chosen id another session owns
  ...
  recordAttempt(commandId: string, generation: number, runtimeRef: string | null, op: string | null = null): AttemptRow {
    ...
    if (pre && pre.cancelRequestedAt != null && !isTerminalState(pre.state)) {
      ...
      // fence comes FIRST (review 05bf2445 — the cancel used to commit before
   ...
   *  recovery tailer commits (review 32f6ed84), so a committed cursor
   ...
   *  insertion ordinal (review 32f6ed84: a receipt from a previous run can
   ...
   *  unacked output (review 37f1ca73: that one is not the replay origin;
