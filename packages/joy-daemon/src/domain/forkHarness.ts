...
export function forkAgyConversation(conversationId: string): string {
  ...
  const { DatabaseSync } = createRequire(import.meta.url)("node:sqlite") as { DatabaseSync: new (p: string) => { prepare(sql: string): { run(...a: unknown[]): unknown }; close(): void } };
...
export function forkPiSession(sessionId: string): string {
  ...
  const lines = readFileSync(join(hit.dir, hit.file), "utf8").split("\n");
