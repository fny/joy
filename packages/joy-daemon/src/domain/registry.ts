...
export class SessionRegistry {
  ...
  // ── boot quarantine (review 05bf2445) ──────────────────────────────────────
  ...
  async create(opts: CreateSessionOpts): Promise<AgentSession> {
    ...
    // (Astra on bc629ac1). Released in the wrapper below.
  ...
  async #createInner(opts: CreateSessionOpts, id: string, cwd: string): Promise<AgentSession> {
    ...
    const matchesRequest = (live: AgentSession): boolean => {
      ...
      // a request for a new launch (Astra on 7848f432).
    ...
    // session in the same folder — Astra on 57fb90a3), a live object under
    ...
    if (opts.id) {
      ...
      // replacement (self-join deadlocked every restart; Astra on bc629ac1).
    ...
    if (opts.id) {
      ...
      // different launch (Astra on 57fb90a3, #44).
  ...
  async #resurrectCodexOrphans(): Promise<void> {
    ...
    for (const rec of listWindowRecords()) {
      ...
      // never touch a handle someone else owns (Astra on 4cff51bf).
