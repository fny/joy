...
import { withDeadline, spawnSyncBounded, retireChildProcess, killProcessGroup, processGroupMembers, pidAlive, withFd, boundedWriter } from "./bounded";
...
describe("killProcessGroup", () => {
  test("a TERM-resistant child that outlives its exited group leader is still found and killed (#571)", async () => {
    ...
    // group and exits immediately — the shape of a launcher whose server
    // child ignores the first signal.
    const leader = spawn("/bin/sh", ["-c", "trap '' TERM; sleep 30 & exit 0"], { detached: true, stdio: "ignore" });
    leader.unref();
    const pgid = leader.pid!;
    for (let i = 0; i < 50 && leader.exitCode === null; i++) await sleep(20);
    expect(leader.exitCode).toBe(0);
    // The leader is gone, but its group is not.
    const before = processGroupMembers(pgid);
    expect(before.length).toBeGreaterThan(0);
    expect(before).not.toContain(pgid);

    const logs: string[] = [];
    const gone = await killProcessGroup(pgid, { graceMs: 300, log: (l) => logs.push(l) });
  ...
  test("an already-dead pid resolves true without escalation", async () => {
    ...
    for (let i = 0; i < 50 && p.exitCode === null; i++) await sleep(20);
