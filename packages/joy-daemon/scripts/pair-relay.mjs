// Programmatic pairing of a joy-daemon daemon against a joy relay's native
// account plane (/joy/v2/auth*). Replicates BOTH sides of the QR flow in one
// process:
//   account side (joy-app):  /joy/v2/auth (sign challenge) -> token; approve the
//     terminal request with encryptBox([0x00||contentPub], cliBoxPub)
//   terminal side (daemon):  /joy/v2/auth/request -> poll -> decrypt -> write
//     access.key {token, encryption:{publicKey, machineKey}} + settings.json
// Usage: node scripts/pair-relay.mjs <relayUrl> <credsDir>   (run from packages/joy-daemon)
...
async function post(path, body, token) {
  const headers = { "Content-Type": "application/json", "X-Joy-Client": "cli/e2e" };
...
const { token: accountToken } = await post("/joy/v2/auth", {
...
const resp = await post("/joy/v2/auth/request", { publicKey: b64(termKp.publicKey), supportsV2: true });
...
// ── 5. write joy-daemon credential files ──
