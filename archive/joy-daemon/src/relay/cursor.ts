...
import { decodeFromWire, eventFromEnvelope } from '../wire/legacyFacade';
...
export class Cursor {
    ...
    async tryLoadSnapshot(): Promise<boolean> {
        ...
        // NB: cap at INT4_MAX — `seq` is a Postgres int4 on happy-server, and
        // passing Number.MAX_SAFE_INTEGER blows up serialization with a 500.
        const PAGE = 100;
    ...
    private parse(m: RawMessage): ParsedEvent | null {
        ...
        const envelope = decodeFromWire(plain, m.id);
        if (!envelope) return null;
        if (!envelope.sessionId) envelope.sessionId = this.opts.sessionId;
        return {
            seq: m.seq,
            envelope,
