        ...
        try {
            switch (req.method) {
                ...
                case 'session.start': {
                    ...
                    if (!p?.agentBin) return err('invalid-params', 'session.start requires agentBin');
                    if (p.mode === 'create') {
                        if (!p.tag || !p.metadata) return err('invalid-params', 'create mode requires tag + metadata');
                        const created = await this.opts.relay.createSession({ tag: p.tag, metadata: p.metadata });
                    ...
                    if (p.mode === 'adopt') {
                        if (!p.sessionId || !p.sessionKeyB64 || !p.variant) return err('invalid-params', 'adopt mode requires sessionId, sessionKeyB64, variant');
                        const s = await this.opts.sessionManager.start({
                    ...
                    return err('invalid-params', `session.start: unknown mode '${(p as { mode?: string }).mode}'`);
                }
                case 'session.stop': {
