...
- **Daemon-recover variant:** with the same session live, restart `joy-daemon` so `recover()` re-adopts it, then open the session in the app. The full prior history must still render on open (the daemon must bind the existing transcript and the app must backfill server history) — no empty-until-first-message state.
