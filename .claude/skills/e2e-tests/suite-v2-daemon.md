...
3. Start the daemon from source on the PRIVATE tmux server (isolation rules
   ...
   `env -u TMUX -u TMUX_PANE TMUX_TMPDIR=/tmp/joy-test-tmux HAPPY_HOME_DIR=$HOME/.joy-test JOY_RELAY_URL=http://127.0.0.1:3105 PORT=4999 TMUX_SESSION=joy-test nohup pnpm -C packages/joy-daemon start > /tmp/joy-test-daemon.log 2>&1 &`
...
the REAL agent's marker answer arriving as durable v2 output events, turn
terminal + message delivered, purge. Exit 0 = pass.
...
- Kill the test daemon (`pkill` on the pid from the log / port 4999 holder)
  and the `joy-test` tmux session on the PRIVATE socket.
