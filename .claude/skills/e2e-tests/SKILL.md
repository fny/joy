...
**Pair the daemon with the browser account (~1 min):** `JOY_HOME_DIR=$HOME/.joy-test JOY_RELAY_URL=http://127.0.0.1:3105 pnpm -C packages/joy-daemon joy auth http://127.0.0.1:3105` prints a pairing key / link; in a browser already logged into the target account open `http://localhost:8082/terminal/connect#key=<key>` and click "Accept Connection"; the daemon writes `access.key` + `settings.json` (with a new machineId) under `$JOY_HOME_DIR/relays/127.0.0.1_3105/`. Then verify RPC health (Setup step 5) before testing.
