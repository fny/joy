...
joy-cli/happy-server/joy-app code.
...
6. joy-cli reports staged → submitted → started → receipted evidence using
...
- **Recovery when the runtime can prove the execution** (for joy-cli: the
...
| Table | Purpose |
|---|---|
| `native_sessions` | id (relay-minted UUID), account, owner daemon, `localSessionId` (joy-cli 8-hex, unique per daemon), creation intent identity, state `provisioning→starting→active/detached/failed/archived`, `next_seq`, revision, active turn, recovery flag, wrapped session key envelope |
...
joy-cli types into Claude Code's TUI and reads back via pane parsing +
...
3. joy-cli native prompt receipt/dispatch/transcript reconciliation.
