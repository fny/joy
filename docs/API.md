...
| `joy-harness-capabilities` | GET /harnesses/capabilities (and `capabilities` in every `GET /v2/harnesses[/:h]` descriptor) | Per harness: `models {pick, source: static\|live\|none, switchLive}`, `effort {levels[], default, perModel, switchLive} \| null`, `permissions {modes[{key,name,description}], default, switchLive} \| null`, `resume {continueLast, byId, pastList, fork}`, `extraArgs: cli\|config\|null`, `fallbackModel`, `resumeLimitMb`. The one table both ends use |
