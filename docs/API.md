...
| `joy-storage` / `joy-storage-nuke` | GET /storage · POST /storage/nuke | What every session leaves under `~/.joy` on the machine — media (`sessions/<id>`), window record, queue file, relay receipts, ledger rows — each with bytes, file count and newest/oldest mtime, plus the shared remainder (ledger file, usage cache, v1 import) and orphaned state files no record names (`domain/footprint.ts`). Nuke `{ids[], killLive?}`: a live session is killed and archived first only with `killLive` (else reported `live` and left alone), then media, queue, receipts, record (via `deleteWindowRecord`, so a refused unlink is tombstoned) and ledger rows go; per-id `{ok, bytesFreed, removed[], error?}`. The relay row is the APP's to delete afterwards (`DELETE /joy/v2/sessions/:id`, which purges its events) — daemon first, so the kill's archive lands on a row that still exists |
