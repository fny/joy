...
| Machines | `GET/POST /machines` · `GET/PATCH/DELETE /machines/:id` | POST upserts (403 `machine_owned_elsewhere`); PATCH is CAS on `metadata`/`daemonState` (`expected*Version` → `success`\|`version-mismatch`); `active`/`activeAt` derived from daemon leases. The sealed `metadata` carries the daemon's `capabilities` advertisement (today `spawnSpecSealed: true`); the app reads it before choosing a wire form, and a daemon that predates it simply advertises nothing |
