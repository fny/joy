...
| `joy-restart-session` | POST /sessions/:id/restart | Relaunch in place (the same conversation and settings: a codex replacement resumes its thread with the persisted model, effort, permission mode AND `-c` config overrides from `extraArgs`, #561) |
