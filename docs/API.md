...
| `joy-create-session` | POST /sessions | Spawn agent (`agent`: claude\|codex\|opencode\|pi\|agy; `resume_id`, `forkSession`, `continue`, `model`, `effort`, `permissionMode`, `yolo`, `extraArgs`, `gitUrl`). **Per harness** (the table in `domain/harnessCapabilities.ts`, served by `joy-harness-capabilities` and inside every `GET /v2/harnesses` descriptor, is what the app renders from and what `registry.create` validates against — an option a harness lacks is refused with a sentence, never silently dropped): `model` — claude a name, codex a catalog `model`, opencode/pi a catalog id `<providerID>/<modelID>` (the old bare curated ids still pass; a model the loaded live catalog does not list is refused), agy a display name; `effort` — claude `CLAUDE_EFFORT`, codex the next turn's effort, opencode the model's reasoning `variant` (sent with the model), pi `--thinking off|minimal|low|medium|high|xhigh|max`, agy `--effort low|medium|high`; `permissionMode` — claude bypassPermissions\|auto\|default\|acceptEdits\|plan, codex default\|read-only\|safe-yolo\|yolo, opencode default\|plan\|yolo (plan = the session's agent set to `plan`; yolo = a per-session allow-all ruleset via `PATCH /session/:id {permission}`; default = build agent, empty ruleset), pi default\|plan (plan = `--tools read,grep,find,ls`), agy bypassPermissions\|acceptEdits\|plan (`--mode`, approvals always skipped in print mode); legacy aliases are normalised first (`bypassPermissions` → codex/opencode `yolo`, pi `default`; `normalizePermissionMode`); `forkSession` + `resume_id` — claude `--fork-session`, codex/pi/agy copy the history under a fresh id first (`domain/forkHarness`) then resume the copy, opencode refused; `extraArgs` — claude appended verbatim to the shell line, pi/agy shell-split (`domain/shellWords`, quotes honoured, control chars refused) and appended to argv, codex `key=value` config overrides, opencode refused; `fallbackModel`/`resume_limit_mb` claude only. pi/agy/opencode launch knobs persist in the window record (`piSettings`/`agySettings`/`opencodeSettings`) so restart/recover relaunch the same way. Clone: `gitUrl` — clone first into cwd, attempt-owned temp dir renamed in on success, failure reported as `clone_failed:<msg>` and nothing launched, also honoured by the relay spawn spec (#151, #547); unknown agent → loud error). `cwd` is canonicalised ONCE (`paths.canonicalCwd`: `~` expanded, then a filesystem walk from the root — every existing component realpath'd and each `..` applied to the PHYSICAL directory reached, so `shortcut/..` is the link target's parent, never the link's; only the not-yet-existing tail folds lexically onto that) before the clone, the launch, the window record and the Claude transcript path, the way Claude Code keys its own project dir (#549 #564). The relay spawn spec's cwd goes through the same step before the clone, the spawn intent and create(), and `cloneForSpawn` returns the canonical checkout path its callers launch in (#549 residual); a record-only recovery (opencode/pi/agy/codex) canonicalises a legacy record's spelling before the existence check, the session and the card, and writes the physical path back (#564 residual). `continue` pins the selected transcript (#550): create() resolves the project's newest transcript, reserves it (a bind claim an in-flight import cannot replace, and one `list()` never settles while the create is in flight — only the create's own finally, `end` and `retire` release it) and launches `claude --resume <that transcript's id>`, so the launch continues exactly the reserved file even when another conversation lands in the project before the Enter; with nothing to select the whole project dir is reserved until the session binds its transcript and plain `--continue` is emitted. Claude gets `CLAUDE_CODE_ENABLE_TASKS=0` + fresh `--append-system-prompt` every spawn. Over the relay the same options travel as the `spawnSpec` of `POST /joy/v2/sessions`, SEALED (#107): `v2e1:` + b64(nonce24 ‖ secretbox(json)) under the machine's dedicated spawn-spec key (`deriveKey(machineKey, 'Joy Spawn Spec', [machineId])`, a sibling of the tunnel key; both ends derive it, the relay never holds it) once the daemon advertises `capabilities.spawnSpecSealed: true` in its sealed machine metadata. Plain JSON `{v:1,t:'spawn',…}` from an app that predates the seal — or one without the machine key — still spawns on any daemon; a sealed spec the daemon cannot open (wrong key, no machine key) is reported `spawn-failed` `bad_spawn_spec:<why>` and nothing launches |
| `joy-restart-session` | POST /sessions/:id/restart | Relaunch in place, carrying the CURRENT model, effort (`currentEffort` after a mid-session `/effort`, #51) and permission mode, and a user-locked `/title` (#474; a `/title` is acknowledged only once its record write lands, so the replacement restores exactly what was acknowledged). A codex replacement also carries its launch config overrides (`extraArgs` → `-c key=value`, #561). |
| `joy-fork-session` | POST /sessions/:id/fork | Fork from the last message into a NEW session (every harness but opencode) → `localSessionId`. Permission mode fails CLOSED (#50): the pane's live mode, else the window record's persisted mode, else `default` — never bypass |
| `joy-handoff` / `joy-handback` | POST /sessions/:id/handoff {agent, model?}, POST /sessions/:id/handback | Hand a session's work to another model via a note the session writes (returns {ok, pending}; progress on the cards' `joy__handoff`); hand it back into the original session. One in-flight job per session: a second call while the note is being written (or a persisted job exists) is `{ok:false, error:"handoff already in progress" \| "handback already in progress"}` (#53). The card is marked `writing` only AFTER the note prompt is durably queued: a refused spool commit returns `{ok:false, error:"not_durable", detail}` (HTTP 503) with the card, queue and job ledger untouched, so the same call can simply be retried; a handback whose target (the note writer) has ended is refused with a sentence, as is a handoff of an ended source |
| `joy-teleport-export` / `joy-teleport-import` | POST /sessions/:id/teleport-export, POST /teleport-import | Move a conversation between machines: export the resumable transcript tail (base64, ≤6MB) with the source's permission mode (same fail-closed rule as fork, #50); import canonicalises `cwd` (#549), writes the transcript under that cwd's project dir and resumes (claude) under a new id; a missing `permissionMode` imports as `default`. A same-machine import is refused when its DESTINATION TRANSCRIPT PATH (`~/.claude/projects/<encoded canonical cwd>/<id>.jsonl`) is one a session here owns — every live session (its bound `transcriptPath`, else its canonical cwd + Claude id) and every record (canonical `launchCwd` + Claude id) contributes the path its conversation lives at; ownership is by path, not folder, because the project-dir encoding is lossy (`project-a` / `project_a` collide, #550 residual). Into a folder whose transcript path differs it is the supported same-box fork (#550). The bytes land in an attempt-owned temp file under the destination's path lock and are renamed into place; an unowned leftover at the destination is set aside and restored if the launch fails, and a file this import created is removed then — a failed import never loses prior bytes |
...
| `joy-send` | POST /send · POST /v2/send | Deliver text to a session (queue-routed); the `/v2/send` alias is the machine-plane form the app and joy-mcp reach over the tunnel for a mid-turn `/steer` (the v2 router answered 404 there until 2026-09-11, so those steers fell back to a relay turn behind the running one). `from` (`joy:<id>` \| `cli` \| `app` \| `cron:<name>`) + optional `replyTo` make the DAEMON wrap the text in `<joy-message from=… reply-to=…>` and stamp `meta.from` on the mirrored record — the sender's own wrapper is stripped, a `joy:` sender must exist here. `replyTo` defaults to `from` for a `joy:` sender; an explicit `null` or `""` stamps NO `reply-to` (what `joy send --no-reply` and `joy run` send, #112). `exclusive` keeps the old refuse-if-busy scripting contract. `ask: true` (what `joy ask` sends) stamps `answer="inline"` and NO `reply-to`: the callee answers in its reply text, never with a `joy send` back (2026-09-11: with a reply-to the asker read the turn's prose while the real answer arrived later as a stray peer message); a `joy:` asker is recorded as waiting on the target's command, and an ask that would close a cycle of open asks (X asks C while C's ask of X is open) is refused `would_deadlock` (409) with the `chain`. Returns `queued_id`. Acceptance is durable: if the queue spool cannot be persisted the op returns `not_durable` (HTTP 503) and nothing is acknowledged or mirrored (#551; same for `queueAdd` and handoff notes) A daemon-owned slash command (`/title`, `/steer`, `/btw`, `/login-code`, `/joy-prompt`) is NOT wrapped whole — its head stays interceptable; a `/steer` / `/btw` body still carries the `<joy-message>` stamp (#552). |
...
| `joy-storage` / `joy-storage-nuke` | GET /storage · POST /storage/nuke | What every session leaves under `~/.joy` on the machine — media (`sessions/<id>`), window record, queue file, relay receipts, ledger rows — each with bytes, file count and newest/oldest mtime and a `kind` (`running` — agent up; `detached` — registry holds it, process gone; `record` — files only), its own tmux server if one answers on `joy-<id>` (panes with pid/command, created/activity) or a stale socket, plus the shared remainder (ledger file, usage cache, v1 import), orphaned state files no record names, and `looseTmux[]` — joy-labelled sockets in the tmux dir (`joy-<id>`, legacy `joy-<relayKey>-s-<id>`) that no session or record owns, live (with panes) or stale (`domain/footprint.ts`). Nuke `{ids[], tmux[]?, killLive?}` — `tmux` are loose labels: kill-server if answering, then the socket file is unlinked; a live session is killed and archived first only with `killLive` (else reported `live` and left alone), then the session's own tmux server first (a record-only session can still have one up, and deleting the record around it would manufacture a loose server), media, queue, receipts, record (via `deleteWindowRecord`, so a refused unlink is tombstoned) and ledger rows go; per-id `{ok, bytesFreed, removed[], error?}`. The relay row is the APP's to delete afterwards (`DELETE /joy/v2/sessions/:id`, which purges its events) — daemon first, so the kill's archive lands on a row that still exists |
| `joy-session-processes` | GET /sessions/:id/processes | Every process under the session's agent in tree order — `{pid, ppid, depth, name, args, cpuPercent, rssBytes, elapsedSeconds}` — plus `totals` and the `roots` sampled (`domain/procStats.ts processTreeList`: on Linux two /proc snapshots 400 ms apart give each pid its own CPU right now, the same method as the info screen's aggregate; macOS is one `ps` call). Rooted at the agent pid; a session without one (detached, record-only) is rooted at its tmux pane shells; 404 `session_not_found` when neither exists |
| (card) `joy__login` | — | `{url?, since, error?, kind?: claude\|codex, code?, expiresAt?}`. Claude: the OAuth URL, and a pasted code goes back via `/login-code`. Codex (`codex/codexPane.ts` watcher): the device-flow URL plus the one-time `code` to enter in the BROWSER, or `error` alone for a dead token ("Your access token could not be refreshed…") with no link. The chooser ("Sign in with ChatGPT / Device Code / API key") is published as `joy__dialog`; the continue screens are pressed through |
...
| `joy-create-session` `headless` | POST /sessions `{headless:true}` (CLI: `joy new --headless`) | Nobody is watching this session: it is kept out of the app's session list and sends no turn-done push (nor the agent's own `<joy-notify>`). A `permission` or `question` push STILL goes out and the session STILL appears while it is blocked — hiding the one case you must act on is how an unattended job wedges unnoticed. Persisted on the window record (`headless`), mirrored on the card as `joy__headless`; the app hides it via `sessionFacts.hiddenFromList` |
| `joy-set-notifications` | POST /sessions/:id/notifications `{muted}` | Silence this session's pushes (turn-done, permission, question and the agent's own `<joy-notify>` tags) on EVERY device. Enforced daemon-side because that is the last point before the relay fans a push out — a phone draws a remote notification before the app is consulted, so a mute applied on receipt would arrive too late. Persisted on the window record (`notificationsMuted`, survives a restart) and mirrored onto the card as `joy__muted` so the app can show the session silenced. Session STATE is untouched: it still goes green when it finishes |
| `joy-set-mode` | POST /sessions/:id/mode | Permission mode on a LIVE session: claude walks the pane (Shift+Tab), codex stores it for the next turn, opencode applies it now (plan ⇄ build agent, yolo ⇄ allow-all ruleset), pi/agy refuse with a sentence (fixed at launch — restart) |
| `joy-set-model` | POST /sessions/:id/model (also `PATCH /v2/sessions/:id {model}`) | Model on a LIVE session, one contract for every harness: opencode switches server-side now (catalog id or bare curated id), codex stores it as the next `turn/start` override (codex keeps it for later turns), claude/pi/agy `{ok:false, error}` with the sentence to show. The session record carries `current_model` |
| `joy-set-effort` | POST /sessions/:id/effort (also `PATCH /v2/sessions/:id {effort}`) | Effort on a LIVE session: codex the next turn's (`low\|medium\|high\|xhigh`), opencode the model's reasoning variant (`"default"` clears), claude/pi/agy refused with a sentence. Records carry `current_effort` and `permission_mode` for codex/opencode/pi/agy |
...
| `joy-codex-models` / `joy-opencode-models` / `joy-pi-models` / `joy-agy-models` | GET /codex/models, /opencode/models, /pi/models, /agy/models (v2: `GET /v2/harnesses/:h/models`) | Model catalogs. codex: its models cache. opencode: LIVE — a short-lived `opencode serve` answers `GET /provider` (connected providers), memoised 10 min, rows `{id: "<providerID>/<modelID>", providerID, modelID, displayName, isDefault, recommended, variants[]}` (variants = the effort levels), the curated fireworks pair marked `recommended` and the fallback when the server cannot be asked. pi: `pi --list-models` parsed, rows `{id: "<provider>/<model id>" (what `--model` takes), providerID, modelID, displayName, isDefault, recommended, thinking}`, same memo/fallback. agy: `agy models` display names — the name is the `--model` id. The app keeps its own per-harness allowlist over these (a fresh one = the recommended rows), so a picker never shows a whole install |
| `joy-harness-capabilities` | GET /harnesses/capabilities (and `capabilities` in every `GET /v2/harnesses[/:h]` descriptor) | Per harness: `models {pick, source: static\|live\|none, switchLive}`, `effort {levels[], default, perModel, switchLive} \| null`, `permissions {modes[{key,name,description}], default, switchLive} \| null` (`default` is the harness's own no-prompts mode: codex/opencode `yolo`, claude/agy `bypassPermissions`, pi `default`; a create without a mode uses it), `resume {continueLast, byId, pastList, fork}`, `extraArgs: cli\|config\|null`, `fallbackModel`, `resumeLimitMb`. The one table both ends use |
| `joy-past-sessions` | GET /past-sessions?harness=&directory= (v2: `GET /v2/harnesses/:h/sessions?directory=`) | Resumable past conversations of ONE harness in ONE directory, newest first: `{id, title, updatedAt, sizeBytes}` — `id` is what `resume_id` takes. claude: transcripts (+ `logTitleFor`); codex: rollouts whose `session_meta.cwd` matches, title = first real prompt in a bounded head; opencode: the server's sessions for the directory; pi: `~/.pi/agent/sessions/<cwd key>/` files (header cwd verified), title = first prompt; agy: `conversation_summaries.db` rows whose `workspace_uris` carry `file://<cwd>`, title \|\| preview. A store that is absent is an empty list (`domain/pastSessions.ts`) |
| `joy-opencode-sessions` / `joy-opencode-set-model` | GET /opencode/sessions, POST /sessions/:id/opencode/model | opencode extras — superseded by `joy-past-sessions` and `joy-set-model`; kept for older apps (`GET /v2/harnesses/opencode/sessions?cwd=` still answers in its original shape) |
...
| (route) | GET /v2/sessions/:id/files/session | The session's own files outside its project — `~/.joy/sessions/<id>/` (uploads the app sent, in `uploads/`; the agent's `media/`; handoff notes) — newest first: `{ ok, root, files: [{ path (absolute), relativePath, name, size, mtimeMs }], truncated }` (500 files, depth 4, symlinks never followed). The paths open through `files/content`, whose read allows the session directory. Uploads land in `uploads/` named as given; a taken name gets a local timestamp before the extension (`report.20260911-081518.pdf`, then `-2`) |
...
|---|---|---|
| `/title <text>` / bare `/title` | all | Set+lock / unlock the session title. Claude: durable FIRST — the title+lock (or the unlock) is written to the window record before anything in memory changes or the relay summary is published; a record write that fails REFUSES the command (`WindowRecordWriteError` → `joy-send` answers `not_durable`, a relay turn fails) and the previous title and lock stand, so a later replacement never reverts an acknowledged title (#474 residual) |
...
| Automations | `GET/POST /automations` · `GET/PATCH/DELETE /automations/:id` · `GET /automations/failures` · `POST/GET /automations/:id/runs` · `POST /automation-runs/:id/report` · `POST /automation-runs/:id/ack` | A folder, a prompt and a trigger, and the runs they produce. `spec` is a SpawnSpec sealed under the TARGET MACHINE's key — the relay stores ciphertext and reads none of it, because the daemon holds a machine key and no account key (which is also why `joy automation create` can only author for its own machine). A run is NOT a new execution mechanism: it calls `core.createSession` with mode `spawn`, so it rides the nucleus lane the daemon already polls, and the run id is the `creationIntentId` — namespaced `automation-run:<id>` so the daemon can tell a run from an ordinary spawn — which makes a retried trigger incapable of spawning twice. Three refusals are RECORDED rather than silent: an overlapping firing is a `cancelled` run with `skipped_overlap`; a spawn that cannot happen leaves the run `failed` with the reason, never `queued` forever; a disabled automation is `409`. Terminal run states are FINAL — a late or retried `report` cannot rewrite one. `PATCH` is CAS on `expectedSpecVersion` (409 `spec_version_mismatch` carrying the current). `/automations/failures` is every failure nobody has dismissed — the sidebar's top tier — and `ack` sets `acknowledged_at` rather than deleting, so the run history keeps it. **Triggers**: `manual`, `automation_done`, `schedule`. Three kinds were removed rather than left dead: `turn_done` and `machine_online` (012) for want of anyone using them, and `session_state` (013) because it could not have worked where triggers are evaluated — the states worth firing on are computed by the APP from sealed card metadata the relay cannot read. If "the agent died, run this" is wanted it belongs as a `session_detached` trigger named for what the relay can actually see — a five-field cron expression carried in the trigger's own `filter`, with a `timezone`, validated at WRITE time (`400 bad_cron` / `bad_timezone`) because an expression accepted now that never fires is an automation nothing explains later. The relay ticks every 30s (`automations.tick`), advances `next_run_at` BEFORE running so a throwing schedule is not retried forever, and answers the catch-up question explicitly: it fires once and records what was missed as a `cancelled` run with `missed_schedule` and a count, rather than replaying a backlog or silently dropping it. Cron is parsed by `src/cron.mjs` — no dependency — which walks a calendar rather than adding fixed days, so a vanished DST hour is skipped and a repeated one fires once (identity is the wall-clock minute). They are evaluated when the relay writes the event, always after the transaction commits, and a session an automation PRODUCED fires no triggers — checked at the relay, since `turn_done` on a run's own turn is an infinite loop and `automation_done` makes cycles trivial to write by accident. Deleting an automation takes its triggers and history and leaves its sessions alone |
| Restore | `GET /restorable` · `POST /restore {ids?}` | Sessions this machine lost to a REBOOT, and the call that brings them back, each resuming its own conversation (`--resume` / the agent's equivalent — claude's transcript uuid, codex's thread, opencode's session, pi's session, agy's conversation). A daemon crash loses none of this: tmux outlives the daemon and `recover()` re-adopts every window; a reboot kills tmux and leaves only the window records. A record whose per-session tmux server still answers is NOT offered (it is adopted, and relaunching it would put two agents in one folder), nor is one with no `socket` (pre-per-session-server, unprobeable). Records are deleted when a session is killed or archived, so a surviving record IS the "was it running" test. Restores run SEQUENTIALLY and each result says what happened to that session, so a partial failure names the folders that did not come back. Deliberately NOT automatic on daemon start — a reboot would otherwise launch a dozen agents that immediately begin work nobody asked for at that moment. Paths are top-level, NOT `/sessions/restorable`: `GET /sessions/:id` is declared earlier and routes match in declaration order, so the literal path lost to the id pattern and the request became a lookup for a session called "restorable" |
...
| Daemon lane | `POST /daemon/leases` · `PUT /daemon/leases/:id` · `…/claims/{work,control,tunnel}` · `…/deliveries/:id/received` · `…/sessions/:id/{bind,spawn-failed,facts}` · `PATCH /daemon/sessions/:id` (card, state, and `sessionKeyEnvelope` — the lane re-envelopes every bound session to the current account content key on boot, so a content-key rotation never strands existing sessions) · `…/turns/:id/{submitted,start,facts,reconcile}` · `…/tunnel/:id/frames` | | Daemon lane | `POST /daemon/leases` · `PUT /daemon/leases/:id` · `…/claims/{work,control,tunnel}` · `…/deliveries/:id/received` · `…/sessions/:id/{bind,spawn-failed,facts}` · `PATCH /daemon/sessions/:id` (card, state, and `sessionKeyEnvelope` — the lane re-envelopes every bound session to the current account content key on boot, so a content-key rotation never strands existing sessions) · `…/turns/:id/{submitted,start,facts,reconcile}` · `…/tunnel/:id/frames` | | Daemon lane | `POST /daemon/leases` · `PUT /daemon/leases/:id` · `…/claims/{work,control,tunnel}` · `…/deliveries/:id/received` · `…/sessions/:id/{bind,spawn-failed,facts}` · `PATCH /daemon/sessions/:id` (card, state, and `sessionKeyEnvelope` — the lane re-envelopes every bound session to the current account content key on boot, so a content-key rotation never strands existing sessions) · `…/turns/:id/{submitted,start,facts,reconcile}` · `…/tunnel/:id/frames` | | Daemon lane | `POST /daemon/leases` · `PUT /daemon/leases/:id` · `…/claims/{work,control,tunnel}` · `…/deliveries/:id/received` · `…/sessions/:id/{bind,spawn-failed,facts}` · `PATCH /daemon/sessions/:id` (card, state, and `sessionKeyEnvelope` — the lane re-envelopes every bound session to the current account content key on boot, so a content-key rotation never strands existing sessions) · `…/turns/:id/{submitted,start,facts,reconcile}` · `…/tunnel/:id/frames` | lease-token authenticated; presence = unexpired lease. Records produced while a relay turn runs post as that turn's `output` facts (fenced to its lease, drained before the terminal fact); records outside any turn post to `…/sessions/:id/facts` (output only, `turnId` null, same replay/budget rules). `spawn-failed` body is `{reason, deliveryId}` — `deliveryId` names the ATTEMPT (#612) and the answer is `{ok, applied, reason}`. The daemon retires the spawn COMMAND only on a positive answer (#581): `applied:true`, or a settled `reason` — `already_bound` / `already_progressed` mean the command moved on. `applied:false` with `stale_attempt` / `ambiguous_attempt` means only this DELIVERY was stale while the spawn command is still live and still queued, and an answer with no application result at all (a null body, `{}`, `{ok:false}`) is an unknown outcome: either way the daemon retires only the delivery and reports again on the command's next offer. Output the relay refuses for good (`429 session_event_budget_exhausted`) is dropped so the turn can still terminalize, and the loss is surfaced rather than silent (#130): the daemon counts it per session, publishes `joy__eventBudget` {since, dropped} on the card, and fires one push at the first refusal. `/submitted` and `/start` answer `409 session_archived` / `409 session_failed` when the session closed under an in-flight turn (#614) — archiving cancels queued turns and supersedes outstanding deliveries in the same transaction; the daemon drops the turn (no retry, no `failed` terminal; an already-dispatching turn is closed `cancelled`). `…/tunnel/:id/frames` answers `404 request_gone` (client left / idle deadline), `403 wrong_daemon` (another machine's lease), `410 client_gone` (the client's socket is already dead) or `429 client_slow` (the client did not drain 8 MiB of buffered response within 10 s — the relay destroyed its stream) instead of a 200 carrying `{error}` (#83); all are settled BEFORE the frame body is read, and the executor treats every one as terminal: it stops streaming, cancels the local response it was reading, and never re-posts. A post may be held up to 10 s while a slow client drains; exchanges run concurrently per request, so one held post stalls only its own exchange `applied:false` splits two ways (#581): `already_bound` / `already_progressed` mean the COMMAND moved on and the daemon retires it; `stale_attempt` / `ambiguous_attempt` mean only this DELIVERY was stale while the spawn command is still live and still queued, so the daemon reports again on its next offer. `…/turns/:id/reconcile` `{resolution: running \| terminal, terminalState?, runtimeEventId?, meta?}` is the post-restart resolution. `running` ADOPTS the turn under the calling lease: an `orphaned` turn, or a live `dispatching`/`running`/`cancelling` turn a PREDECESSOR epoch of the same daemon left behind — the turn re-fences to the caller's epoch instead of being terminalized, its cancellation state is preserved (a cancel-requested turn answers `cancelling`), a turn that was submitted but never started gets the `/start` bookkeeping there (`started_at`, run token, prompt command `applied`, one `turn.started` event carrying `runtimeEventId`, so a later `/start` under the same id is a replay), and the answer is `{turnId, state, adopted: true}`; a turn already under the caller's epoch answers its current state with `replay: true`, a terminal turn answers `{state: "terminal", terminalState, replay: true}`, and a turn nobody's predecessor holds is `409 turn_not_orphaned` (`403` for another daemon's session, `409 session_archived`/`session_failed` for a closed session, `409 another_turn_active`). Adopting a turn that was never started IS its first start, so it takes the first-start event-budget decision `/start` takes (#613): a session whose log is full commits the turn `failed` (`meta.reason: session_event_budget_exhausted`, delivery superseded, no `turn.started`) and answers `409 session_event_budget_exhausted`; a repeat adoption then replays the terminal. Re-fencing a turn that already started is not gated — its start was authorized under the predecessor's budget and its output is refused per fact as always. `terminal` is accepted for an orphaned turn or by the owner epoch releasing a turn it has no worker for (#74). The daemon consults its LEDGER before any orphan cleanup: a turn whose command is still pending locally is never interrupted by the boot pass or the sweep — its resumed loop adopts it once the new driver generation confirms the runtime running (the sweep adopts one orphaned later, mid-loop), a pending cancel is honoured locally, and a `/start` refused with a recovery `409` (`turn_terminal`, `no_current_delivery`, `turn_orphaned_reconcile_first`) is answered by this reconcile, never by cancelling a running agent: a relay terminal that was a cancellation is honoured; any other closes nothing locally — the runtime's own outcome posts as the terminal fact when it ends (a replay on the relay). Only `turn_cancelled`, `session_archived`/`session_failed` and `session_event_budget_exhausted` — from `/start` or from the adoption itself — cancel the local prompt. A reconcile the relay cannot ANSWER (5xx, timeout, transport) is unresolved, not a verdict: the lane retries the adoption with a bounded backoff (1s, 2s, 4s, 8s), then parks the turn `adoption_pending` (visible in the lane handle's `relayTurns()`), posts no `/start` for it, keeps the runtime-confirmed command running and retries every 30s (the orphan sweep adopts it too); the command's own outcome posts as the terminal fact whenever it ends. Every ANSWER to that retry resolves the parked wait, whatever it says: `running` / `cancelling` adopt the turn (a cancel pending is honoured locally once), a terminal the relay already holds is honoured once — a `cancelled` cancels the local prompt, any other terminal (`completed`/`failed`/`interrupted`) is the turn's outcome and closes nothing locally: no re-adoption, no `/start` for a closed turn, the runtime's own outcome posts as the terminal fact when it ends (a replay on the relay). The `adoption_pending` marker clears on every resolved answer and the loop returns to one fresh bounded wait for the command's terminal; only a reconcile the relay could not answer keeps it parked (Astra, F21: clearing the marker for `running` alone re-asked a closed turn in a burst until the then 30-minute turn cap). There is no turn cap: a running turn is never ended by a timer. A turn with no output for 30 min (`turnStallMs`) is REPORTED — `joy__stalled` {since, silentForMs} merged into its card, cleared when output resumes or the turn ends — and left to run; the daemon cannot tell a long tool call from a hung one, so it does not act. Each real wedge is closed by the fact that is already true: the pane-idle tie-breaker (completion lost), the detached edge (process gone), `dispatch_timeout` (never delivered — the row is cancelled, nothing is interrupted), and the `no_local_worker` reconcile. A relay turn whose command the ledger already closed but whose terminal outbox row never committed (a crash between the two) has that row DERIVED from the command — its state and terminal reason, under the stable `term:<turn>` id — before any orphan cleanup on boot, so the boot never publishes an invented `interrupted` ahead of the real `completed`. The derivation walks the ledger by durable command ownership across EVERY session the daemon can name — live handles, window records, and the relay's own rows for this daemon — not the live handles alone (Astra, F21): a session that exists only as a record (its window died with the daemon) keeps its saved outcome, and the boot's record-only archive waits for it — while the session still has an unacked terminal row the archive is owed to the retry loop (an `archive_relay_row` job, 2s…60s backoff) and lands only after the saved terminal is acked, never ahead of it or instead of it. The orphan sweep consults the ledger for a dead session the same way (local, no relay read) before leaving it to the archive. The count is committed in the daemon's ledger INSIDE the transaction that settles the refused outbox row (the drop and its evidence land together, or neither does and the row is retried — an unclean exit right after the refusal cannot erase it) and merged into every card the daemon publishes for that session — a live holder's merge, the boot-time archive of a session with a record but no runtime, an owed archive job — so the warning depends on neither a surviving adapter nor the one-second timer that only coalesces the card PATCH. Two rules close the remaining gaps: a persisted count whose card is behind it (an already-archived row with a clean card, as an upgrade finds it) is repaired by the boot pass — the row's own card re-published with the count merged, other metadata preserved, once (the accepted PATCH is marked on the job row) — and a budget record the ledger cannot READ is an unknown outcome, never an absent budget: no card is published without it, the archive is owed to the retry loop with backoff, the condition is logged once, and the first read that succeeds publishes the count; the app keeps the field on decode and renders it as a persistent banner in the session view and a marker on the sidebar row, naming the recovery (start a new session) — the push is a courtesy, not the warning. |
