...
Normative (RFC 2119). Companion to **`comm-layer-spec.md`**, **`joy-daemon-spec.md`**, **`joy-agent-spec.md`**. Specifies the **joy-daemon** package only. "§N" unqualified = a section of `comm-layer-spec.md`.
...
joy-daemon is the thin user/operator entrypoint. It does **not** run agents and does **not** talk to the relay. Its job is: parse the command, **ensure a joy-daemon is up (launch it if not)**, connect to that daemon over the local control channel, issue the command, and render results. joy-daemon is a **control client of joy-daemon** — not an agent, not a relay client, not a consumer the daemon fans out to.
...
- Auth/pairing/QR, sandbox runtime, interactive local‑terminal mode: **out of the three‑package scope** unless explicitly pulled in later; not part of joy-daemon's core mandate.

---

## 4. Boundaries

### 4.1 joy-daemon ↔ joy-daemon
The local control channel only (joy-daemon spec §4.2). Every session effect is expressed as an input event the daemon appends and a projection the daemon folds and reports back. joy-daemon holds no authoritative state; on reconnect it re‑reads the daemon's projection. Protocol‑version handshake mandatory.

### 4.2 joy-daemon ↔ joy-agent
**None.** joy-daemon never contacts joy-agent. All agent interaction is mediated by joy-daemon.

### 4.3 joy-daemon ↔ relay / apps
**None.** joy-daemon does not connect to the relay. Apps are unrelated readers of the relay log.
...
*joy-daemon is a thin operator front‑end: ensure a daemon, speak its control channel, author input events, render its projection. It owns no protocol and no agent.*
